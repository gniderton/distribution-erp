...
async function upgrade() {
    const sql = `
CREATE OR REPLACE FUNCTION public.create_purchase_invoice(p_vendor_id bigint, p_po_id bigint, p_vendor_invoice_number text, p_invoice_date date, p_received_date date, p_total_net numeric, p_tax_amount numeric, p_grand_total numeric, p_lines_json jsonb, p_parent_id bigint DEFAULT NULL::bigint)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        -- Create Inventory Batch [MODIFIED v15: Added net_purchase_rate]
        ...
        SELECT
            ...
            -- Forensic Math: unit_net = (Line Total Net Taxable) / Accepted Qty
            ((v_line->>'amount')::numeric - (v_line->>'tax_amount')::numeric) / NULLIF((v_line->>'accepted_qty')::numeric, 0),
...
$function$
    ...
    try {
        ...
        console.log('🛡️ GRN ENGINE UPGRADED (v15): Now capturing Net Purchase Rate per batch.');
    } catch (e) {
