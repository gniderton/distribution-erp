...
class KDKEwayBillService {
    ...
    async mapInvoiceToKDK(invoiceId, tripVehicleNumber) {
        ...
        const payload = {
            ...
            vehicleType: 'R',
            mainHsnCode: inv.items[0]?.hsn_code ? (inv.items[0].hsn_code.toString().length === 7 ? '0' + inv.items[0].hsn_code.toString() : inv.items[0].hsn_code.toString()) : "",
            itemList: inv.items.map((item, idx) => ({
                itemNo: idx + 1,
                productName: item.product_name,
                productDesc: "",
                hsnCode: item.hsn_code ? (item.hsn_code.toString().length === 7 ? '0' + item.hsn_code.toString() : item.hsn_code.toString()) : "",
