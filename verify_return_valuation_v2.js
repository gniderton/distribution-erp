...
async function runTest() {
    console.log('--- Starting Refined Return Valuation Verification ---');
    ...
    try {
        ...
        const invNo = 'HIST-' + floor(random() * 1000000);
        ...
        const batchStatusRes = await pool.query(`
            SELECT id, status, is_active, quantity_remaining 
            FROM inventory_batches 
            WHERE product_id = $1 AND batch_code = $2
        `, [productId, batchCode]);

        console.log('\nInventory Batch States:');
        console.table(batchStatusRes.rows);
