...
let output = '';
output += `Original version has ${pages1.length} pages.\n`;
output += `Current version has ${pages2.length} pages.\n`;
...
output += `Missing pages in Current (present in Original): ${JSON.stringify(missingPages)}\n`;
output += `Extra pages in Current (not in Original): ${JSON.stringify(extraPages)}\n`;
...
pages1.forEach(p1 => {
  ...
  if (missingWidgets.length > 0 || extraWidgets.length > 0) {
    ...
    if (missingWidgets.length > 0) {
      output += `  - Missing widgets (present in Original, missing in Current):\n`;
      missingWidgets.forEach(name => {
        const w = p1.widgets.find(x => x.widgetName === name);
        output += `    * ${name} (${w.type})\n`;
      });
    }
    if (extraWidgets.length > 0) {
      output += `  - Extra widgets (present in Current, missing in Original):\n`;
      extraWidgets.forEach(name => {
...
fs.writeFileSync(path.join(__dirname, 'comparison_output.txt'), output, 'utf8');
console.log("Comparison saved to scratch/comparison_output.txt");
