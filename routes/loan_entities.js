...
router.post('/', async (req, res) => {
    const { entity_name, entity_type, role_type, contact_number, email, address, notes, is_active } = req.body;
    try {
        const result = await pool.query(`
            INSERT INTO loan_entities (
                entity_name, entity_type, role_type, contact_number, email, address, notes, is_active
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(`
            ...
            WHERE id = $9
            RETURNING *
        `, [entity_name, entity_type, role_type, contact_number, email, address, notes, is_active, id]);
