...
// ==========================================
// CUSTOMER VERIFICATION STAGING (ONBOARDING)
// ==========================================
...
// 2. GET /api/verify-requests - Admin lists pending requests
router.get('/verify-requests/pending', async (req, res) => {
    try {
        const result = await pool.query(`
...
router.post('/verify-requests/:id/approve', async (req, res) => {
    const client = await pool.connect();
    try {
        const { id } = req.params;
        ...
        // Mark staging request as Approved
