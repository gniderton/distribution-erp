    ...
    try {
        ...
        for (const line of lines) {
            const stockRes = await client.query(`
                SELECT SUM(quantity_remaining) FROM inventory_batches 
                WHERE product_id = $1 AND quantity_remaining > 0 
                AND is_active = true AND status = 'Good' 
                AND (expiry_date IS NULL OR expiry_date >= CURRENT_DATE)
            `, [line.product_id]);
...
// @route   GET /api/sales/invoice-lines
...
router.get('/invoice-lines', async (req, res) => {
    try {
        ...
        if (customer_id && customer_id !== 'all' && customer_id !== '') {
            queryParams.push(customer_id);
            whereConditions.push(`si.customer_id = $${paramIdx}`);
            paramIdx++;
        }

        if (start_date) {
            queryParams.push(start_date);
            whereConditions.push(`si.invoice_date >= $${paramIdx}::date`);
            paramIdx++;
        }
        ...
        console.error("Fetch Sales Invoice Lines Margin Error:", err.message);
        res.status(500).json({ error: err.message });
    }
});
