...
export const generateLoanLedgerPDF = (title: string, ledgerData: any, brandData?: any) => {
  try {
    ...
    const drawMainHeader = (currentPage: number, totalPages: number) => {
      ...
      doc.setFont("helvetica", "bold")
      doc.setFontSize(16)
      doc.text("LOAN LEDGER STATEMENT", pageWidth / 2, headerY + 15, { align: "center" })
      doc.setFontSize(11)
      doc.text(title, pageWidth / 2, headerY + 30, { align: "center" })
    ...
    const fileName = `Ledger_${safeTitle}_${formatDate(new Date().toISOString()).replace(/\//g, '')}.pdf`
    doc.save(fileName)
...
export const exportLoanLedgerToExcel = (title: string, ledgerData: any) => {
  try {
    ...
    csvContent += "LOAN LEDGER STATEMENT\n"
    csvContent += "Ledger Of:," + escapeCSV(title) + "\n"
    ...
    const safeTitle = title.replace(/[^a-zA-Z0-9]/g, '_')
    const fileName = "Ledger_" + safeTitle + "_" + formatDate(new Date().toISOString()).replace(/\//g, '') + ".csv"
