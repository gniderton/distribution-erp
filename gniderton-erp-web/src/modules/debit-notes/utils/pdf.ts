...
function _drawSimpleBox(doc: any, x: number, y: number, width: number, height: number, rows: [string, string][]) {
...
] = Number(existing['Gross (dnHeader: any) {
  try {
    // 1. Fetch Items
    const { data: dnLines } = await api.get(`/api/debit-notes/${dnHeader.id}/items`)
...
] || 0) + Number(line['Gross (dnHeader: any) {
  try {
    // 1. Fetch Items
    const { data: dnLines } = await api.get(`/api/debit-notes/${dnHeader.id}/items`)
...
] = Number(existing['Taxable (dnHeader: any) {
  try {
    // 1. Fetch Items
    const { data: dnLines } = await api.get(`/api/debit-notes/${dnHeader.id}/items`)
...
] || 0) + Number(line['Taxable (dnHeader: any) {
  try {
    // 1. Fetch Items
    const { data: dnLines } = await api.get(`/api/debit-notes/${dnHeader.id}/items`)
...
] = Number(existing['GST (dnHeader: any) {
  try {
    // 1. Fetch Items
    const { data: dnLines } = await api.get(`/api/debit-notes/${dnHeader.id}/items`)
...
] || 0) + Number(line['GST (dnHeader: any) {
  try {
    // 1. Fetch Items
    const { data: dnLines } = await api.get(`/api/debit-notes/${dnHeader.id}/items`)
...
] = Number(existing['Net (dnHeader: any) {
  try {
    // 1. Fetch Items
    const { data: dnLines } = await api.get(`/api/debit-notes/${dnHeader.id}/items`)
...
] || 0) + Number(line['Net (dnHeader: any) {
  try {
    // 1. Fetch Items
    const { data: dnLines } = await api.get(`/api/debit-notes/${dnHeader.id}/items`)
