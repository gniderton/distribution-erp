-- Migration 206: Transit Auto-Knockoff (Merged with Payments/Debit Notes)
-- 
-- Drops the old signature and creates the unified create_purchase_invoice
-- that includes Transit Stock Knockoff, Debit Note Knockoff, and Payment Knockoff.
...
DECLARE
    ...
    -- Transit Knockoff Variables
    v_remaining_incoming numeric;
    v_neg_batch RECORD;
    v_knockoff_amt numeric;
BEGIN
    -- 0. Fetch GST Details
    ...
    LOOP
        ...
        v_remaining_incoming := (v_line->>'accepted_qty')::numeric;

        -- [NEW] Transit Auto-Knockoff Logic
        FOR v_neg_batch IN 
            SELECT id, quantity_remaining 
            FROM inventory_batches 
            WHERE product_id = (v_line->>'product_id')::bigint 
              AND quantity_remaining < 0 
            ORDER BY created_at ASC 
            FOR UPDATE
        LOOP
            IF v_remaining_incoming <= 0 THEN
                EXIT;
            END IF;

            -- Absolute value of negative batch
            v_knockoff_amt := LEAST(ABS(v_neg_batch.quantity_remaining), v_remaining_incoming);
            
            -- Sync old batch metadata and apply knockoff
            UPDATE inventory_batches 
            SET quantity_remaining = quantity_remaining + v_knockoff_amt,
                batch_code = (v_line->>'batch_number'),
                expiry_date = (v_line->>'expiry_date')::date,
                mrp = (v_line->>'mrp')::numeric,
                purchase_rate = (v_line->>'rate')::numeric,
                net_purchase_rate = (v_line->>'net_purchase_rate')::numeric
            WHERE id = v_neg_batch.id;

            -- Log traceability
            INSERT INTO stock_traceability (
                batch_id, product_id, quantity_change, transaction_type, reference_id, reference_type, notes
            ) VALUES (
                v_neg_batch.id, (v_line->>'product_id')::bigint, v_knockoff_amt, 'TRANSIT-RECONCILIATION', v_header_id, 'Purchase Invoice', 'Auto-knockoff against GRN ' || v_internal_id
            );

            v_remaining_incoming := v_remaining_incoming - v_knockoff_amt;
        END LOOP;

        -- Create Inventory Batch for any remaining qty
        IF v_remaining_incoming > 0 THEN
            ...
            SELECT
                ...
                v_remaining_incoming, v_remaining_incoming,
            ...
            INSERT INTO stock_traceability (
                ...
                v_batch_id, (v_line->>'product_id')::bigint, v_remaining_incoming, 
