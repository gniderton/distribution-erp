-- ============================================================
-- Migration 195: Fix create_purchase_invoice Function Ambiguity
-- 
-- Root Cause: Multiple migrations (009, 032, 036, 038, 040, 041, 042, 112)
-- created slightly different overloads of create_purchase_invoice.
-- The DEFAULT NULL vs DEFAULT NULL::bigint difference caused Postgres to
-- treat them as distinct function overloads. When Node.js passes $10=null,
-- Postgres cannot resolve the ambiguity and throws:
--   "function create_purchase_invoice(...) is not unique"
--
-- Fix: Drop ALL existing overloads and recreate the single canonical
-- version (from migration 112) with fully explicit types.
-- ============================================================

-- Step 1: Drop all known overloads (by parameter signature)
-- The CASCADE ensures dependent objects (permissions, etc.) are rebuilt.
...
-- Step 2: Recreate the single canonical version (from migration 112)
-- All parameters are fully typed. p_parent_id has explicit DEFAULT NULL::bigint
-- to prevent future overload ambiguity.
