-- Fix: Include net_purchase_rate but STICK to Product Master for selling rates (Distributor/Wholesale etc.)
...
BEGIN
    ...
    IF v_vendor_gst IS NOT NULL AND LENGTH(v_vendor_gst) >= 2 THEN
        ...
        EXCEPTION WHEN OTHERS THEN v_vendor_state_code := 0; END;
    END IF;

    IF v_vendor_state_code = v_company_state_code THEN v_is_intra_state := true; END IF;
    ...
    SELECT prefix, current_number INTO v_prefix, v_next_num FROM document_sequences WHERE document_type = 'GRN' FOR UPDATE; 
    ...
    INSERT INTO purchase_invoice_headers (
        vendor_id, purchase_order_id, invoice_number, vendor_invoice_number, vendor_invoice_date, received_date,
        total_net, tax_amount, grand_total, status, parent_invoice_id, created_by,
        taxable_amount, cgst_amount, sgst_amount, igst_amount, place_of_supply
    )
    VALUES (
        p_vendor_id, (CASE WHEN p_purchase_order_id = 0 THEN NULL ELSE p_purchase_order_id END), 
        v_internal_id, p_invoice_number, p_invoice_date, p_received_date,
        p_total_taxable, p_tax_amount, p_grand_total, 'Verified', p_parent_invoice_id, 1,
    ...
    LOOP
        ...
        VALUES (
            v_header_id, (v_line->>'product_id')::bigint, (v_line->>'ordered_qty')::numeric, (v_line->>'accepted_qty')::numeric,
            (v_line->>'rate')::numeric, (v_line->>'discount_percent')::numeric, COALESCE((v_line->>'discount_amount')::numeric, 0),
            (v_line->>'scheme_amount')::numeric, (v_line->>'tax_amount')::numeric, (v_line->>'amount')::numeric
        )
        ...
        INSERT INTO inventory_batches (
            ...
            quantity_initial, quantity_remaining, expiry_date, is_active
        )
        SELECT
        ...
        VALUES (v_batch_id, (v_line->>'product_id')::bigint, (v_line->>'accepted_qty')::numeric, 'IN', v_header_id, 'Purchase Invoice', 'GRN Inwarding');
    END LOOP;
    ...
    IF v_diff != 0 THEN
        IF v_diff > 0 THEN v_ledger_lines := v_ledger_lines || jsonb_build_object('code', v_acc_rounding, 'debit', 0, 'credit', v_diff);
        ELSE v_ledger_lines := v_ledger_lines || jsonb_build_object('code', v_acc_rounding, 'debit', ABS(v_diff), 'credit', 0); END IF;
    END IF;

    PERFORM create_journal_entry(p_invoice_date, 'GRN Inwarding: ' || v_internal_id, 'GRN', v_header_id, v_ledger_lines);

    FOR v_dn IN 
        SELECT dn.id, (dn.amount - COALESCE(dna.applied, 0)) as unallocated_credit
        FROM debit_notes dn
        LEFT JOIN (SELECT debit_note_id, SUM(amount) as applied FROM debit_note_allocations GROUP BY debit_note_id) dna ON dn.id = dna.debit_note_id
        WHERE dn.vendor_id = p_vendor_id AND dn.status = 'Approved' AND (dn.amount - COALESCE(dna.applied, 0)) > 0.01
    ...
    LOOP
        ...
        IF v_apply > 0 THEN
            INSERT INTO debit_note_allocations (debit_note_id, purchase_invoice_id, amount) VALUES (v_dn.id, v_header_id, v_apply);
